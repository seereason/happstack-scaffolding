...
The writer portion hold the tuple, ([HJScript ()],[XMLGenT m (HSX.XML
...
newtype Widget m a =
  Widget { unWidget :: RWS () ([HJScript ()],[XMLGenT m (HSX.XML m)]) (Id, Int) a }
...
runWidget :: String -> Int -> Widget m a -> (a, ([HJScript ()],[XMLGenT m (HSX.XML m)]))
...
mapWidget :: ((a, (Id, Int), ([HJScript ()], [XMLGenT m (HSX.XML m)])) -> (b, (Id, Int), ([HJScript ()], [XMLGenT n (HSX.XML n)]))) -> 
             Widget m a ->
             Widget n b
...
html ::  XMLGenT m (HSX.XML m) -> Widget m ()
html xml =
  Widget $ tell ([], [xml])
...
plugW :: ([XMLGenT m (HSX.XML m)] -> [XMLGenT m (HSX.XML m)]) -> Widget m a -> Widget m a
...
input' :: (XMLGenerator m, Functor m, NextId (Widget m))
          => (Id -> XMLGenT m (HSX.XML m)) -- ^ takes a unique id and generates the HTML
...
onReadyXML :: (XMLGenerator m) => [HJScript ()] -> XMLGenT m (HSX.XML m)
...
--    , JShow a
    , Show a
    , JType a
    , JSONCast a) => String -> Widget m (In String, Out a) -> Form v i  [String] ([HJScript ()],[XMLGenT m (HSX.XML m)]) (AsHaskell a)
...
widgetToForm :: 
    ...
    , JShow a
    , Show a
    , JType a) => Widget m (Out a) -> Form ([HJScript ()],[XMLGenT m (HSX.XML m)]) v a
widgetToForm widget =
